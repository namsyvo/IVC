...
func SetPara(read_len, info_len int, max_ins int, err_rate, mut_rate float32) *ParaInfo {
	...
	para_info.Prob_thres = 100 * math.Log10(1 - err) + float64(para_info.Read_len - para_info.Dist_thres) * math.Log10(EPSILON) + 
							float64(para_info.Dist_thres) * math.Log10(1 - 3 * EPSILON)
