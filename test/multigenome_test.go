...
func TestMultiGenomeBuild(t *testing.T) {
	...
	isc.SaveMultigenome("../test_data/refs/genomestar.txt", genome)
	isc.SaveSNPLocation("../test_data/refs/SNPLocation.txt", SNP_array)

	saved_genome := isc.LoadMultigenome("../test_data/refs/genomestar.txt")
	saved_SNP_array, saved_SameLen_SNP := isc.LoadSNPLocation("../test_data/refs/SNPLocation.txt")
